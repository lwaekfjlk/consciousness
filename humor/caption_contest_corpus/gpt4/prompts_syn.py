...
For the relationship between the cartoon description and the caption choice, there are three categories:
(1) cartoon description and caption choice are almost unrelated, little information is shared, almost no extra information can be understood when we read them together.
(2) cartoon description and caption choice are related, extra information can be understood when we read them together.
(3) cartoon description and catpion choice are very related, most information is shared, almost no extra information can be understood when we read them together.'''

_PROMPT_USER_MATCHING_DIRECT_ANSWER_V1 = '''I will describe a New Yorker cartoon to you. Then, I will give you 5 choices (labelled A-E) for captions. One of the captions was the winning caption for that cartoon, the other captions do not correspond to this cartoon. Your job is to first reason step-by-step about which answer might be correct. For the first step, you need to find caption choices that are with (2) relationship with cartoon description, which means that they are related and extra information can be understood when reading them together. Secondly, you need to select one best caption choice among them. In the end, respond with "Answer: X" where X is either A, B, C, D, or E.'''
