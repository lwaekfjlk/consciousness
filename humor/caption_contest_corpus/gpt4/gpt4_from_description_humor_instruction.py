...
def main():
    ...
    if not os.path.exists('query_cache_answer'):
        os.makedirs('query_cache_answer')

    if args.shots == 0:
        if args.fd_mode == 4: # default
            cache = 'query_cache_humor_instruction/{}_{}_cache.jsonl'.format(args.task, args.engine)
        else:
            cache = 'query_cache_humor_instruction/{}_{}_mode={}_cache.jsonl'.format(args.task, args.engine, args.fd_mode)
    else:
        if args.fd_mode == 4:
            cache = 'query_cache_humor_instruction/{}_{}_{}_cache.jsonl'.format(args.task, args.engine, args.shots)
        else:
            cache = 'query_cache_humor_instruction/{}_{}_{}_mode={}_cache.jsonl'.format(args.task, args.engine, args.shots, args.fd_mode)
