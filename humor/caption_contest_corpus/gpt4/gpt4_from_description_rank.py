...
def generate_request_matching_few_shot(query, few_shots):
    ...
    for idx, fs in enumerate(few_shots):
        rank = fs['rank']
        rank_string = ['ABCDE'[r] for r in rank][::-1]
        rank_string = '>'.join(rank_string)

        extra = 'Thanks! How about this one?\n' if idx != 0 else (_PROMPT_USER_MATCHING_DIRECT_ANSWER_V1 + '\n')
        messages.append({
            'role': 'user',
            'content': extra + '\n{}\nChoices:\n{}\n\nWhich of the 5 options (A, B, C, D, or E) is the best caption that truly corresponds to the cartoon and what is the rank for all options?'.format(
                format_chatgpt_input(fs),
                '\n'.join(['{}: {}'.format(cidx, c) for cidx, c in zip('ABCDE', fs['caption_choices'])]))})
        messages.append({
            'role': 'assistant',
            'content': 'Answer: {}, Rank: {}'.format(fs['label'], rank_string)})
...
def generate_request_matching(query, few_shots=None):
    ...
    messages.append({
        ...
        'content': ('OK. Here is a description of the cartoon followed by the five choices.'
                    '\n\n{}\n\nChoices:\n{}\n\nWhich of the 5 options (A, B, C, D, or E) is the best fit? Think step-by-step and finish your response with "RANK: X1>X2>X3>X4>X5" where X1, X2, X3, X4, X5 are either A, B, C, D, or E.'.format(
                        format_chatgpt_input(query),
                        '\n'.join(['{}: {}'.format(cidx, c) for cidx, c in zip('ABCDE', query['caption_choices'])])))})
...
def main():
    ...
    # TODO(haofeiyu): need to change back to train after testing that
    train_instances = list(load_dataset("jmhessel/newyorker_caption_contest", split_name)['train'])
    with jsonlines.open('../clip/task=matching~split=5~valacc=0.64962~pad=1~model=ViT-L*14@336px~lr=5e-06~results_train_set.json', 'r') as f:
        clip_dataset = [x for x in f]
    for idx, (d, clip_d) in enumerate(zip(train_instances, clip_dataset)):
        assert d['image_description'] == clip_d['image_description']
        s = clip_d['logits']
        rank = sorted(range(len(s)), key=lambda k: s[k])
        train_instances[idx]['rank'] = rank
